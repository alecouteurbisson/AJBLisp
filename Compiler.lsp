     ...
     (if (lambdap (car expr))
           (progn
             ...
             (let ((sub (compile-lambda (cdar expr))))
                  (emit :vm-call sub) ))
           (case (car expr)
             (car    (compile-unary :vm-car (cdr expr)))
             (cdr    (compile-unary :vm-cdr (cdr expr)))
             (+      (compile-binary :vm-add :vm-nop (cdr expr) 0))
             (-      (compile-binary :vm-sub :vm-neg    (cdr expr) 0))
             (*      (compile-binary :vm-mul :vm-nop (cdr expr) 1))
             (/      (compile-/ (cdr expr)))
             (not    (compile-unary :vm-not (cdr expr)))
             (and    (compile-binary :vm-and :vm_nop (cdr expr) t))
             (or     (compile-binary :vm-or  :vm-nop (cdr expr) nil))
...
(defun compile-unary(op arg)
  (when (cdr arg)
        (error 1 arg "bad args to unary operator"))
  (compile (car arg))
  (emit op) )
  ...
  (cond
    ...
    ((null (cdr args))
     ...
     (if (not (eq :undefined op1))
         (if (not (eq op1 :vm-nop)) (emit op1))
         (error 1 op "Invalid unary operator") ))
    (t  (compile (pop args))
