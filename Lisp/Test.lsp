...
(defun type-ops ()
  ...
  (slet
    ((vect #(2 2))
     (types (list nil 'a '(a b c) 0 1 0.0 1.0 5 5.0 -1 -1.0 "Hi" vect list let :stdin :create ':err_num_args)) )
    ;
...
(defun application ()
  ...
  (when (eq test-parse :undefined)
    (load "c:\\program files\\ajblisp\\test-parse.lsp"))
  (test-parse expressions verbose) )
