...
(setq *bq-list*        'BQ-LIST
      *bq-append*      'BQ-APPEND
      *bq-list**       'BQ-LIST*
      *bq-nconc*       'BQ-NCONC
      *bq-clobberable* 'BQ-CLOBBERABLE
      *bq-quote*       'BQ-QUOTE
      *bq-quote-nil*    (list *bq-quote* nil) )
        ...
        (t
           (let ((p x) (q))
             (loop
               ...
               (when (eq (car p) comma)
                 (unless (null (cddr p)) (error 1 p "Malformed ,var"))
                 (break (cons *bq-append*
                              (nreconc q (list (cadr p))))))
               (when (eq (car p) comma-at)
                 (error 1 p "Dotted ,@var"))
      ...
      (setq result
         (cond ((atom (car args))
               ...
               ((eq (caar args) *bq-clobberable*)
                (bq-attach-append *bq-nconc* (car (cdar args)) result))
               (t (bq-attach-append *bq-append*
                                    (car args)
                                    result))) )
      (pop args)
      (until (null args) result) )))
...
// The Common Lisp Compatibility Suite
(setq ~car car ~cdr cdr ~append append)

(defun car (x) (if x (~car x) nil))

(defun cdr (x) (if x (~cdr x) nil))

(defun nreconc (x y) (~append (reverse x) y))
