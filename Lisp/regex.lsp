...
(defun compile (state defn)
  (cond
    ...
    ;; Compile alternation
    ((eq '| (car defn))
     (compile-alternate state (cdr defn)) )
...
;; Compile alternation
(defun compile-alternate (state alt)
  (let ((next (new-state)))
       (loop
         (while alt next)
...
;;; each having a non-terminal name as car and the
;;; corresponding text as cdr.
...
;;; A parse error will always be the last entry in the
;;; list.
(defun run-dfa (str)
  (let ((ch (chars str))
        (state 1)
